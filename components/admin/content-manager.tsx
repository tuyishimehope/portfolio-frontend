...
export default function ContentManager({ kind, create = false }: { kind: "projects" | "posts"; create?: boolean }) {
  ...
  return <>
    ...
    {editor ? <Editor key={editor.id} entry={editor} project={project} onSave={save} onCancel={() => setEditor(null)} /> : <>
...
function Editor({ entry, project, onSave, onCancel }: { entry: Entry; project: boolean; onSave: (entry: Entry) => boolean; onCancel: () => void }) {
